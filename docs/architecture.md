...
**You never write credentials into your code.** You never pass a key file path
in your R or Python scripts. The libraries handle discovery transparently.
...
**The rule**: if a value would go in `.env.example`, it will go in
Secret Manager in production. If a value is not in `.env.example`, it should
not be referenced in your code.
...
The practical consequence: **if you need a new package, you cannot just
`install.packages()` inside the container during a pipeline run.** The container
is rebuilt from the lock file every time. Add the package to the base image
repo through a pull request instead.
